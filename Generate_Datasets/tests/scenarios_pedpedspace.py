...
def test_separator():
    ...
    space = [np.array([(i, i) for i in np.linspace(-1, 4.0)]),]
    ...
    # visualize
    with visualize(states, space, './docs/separator_test.mp4') as ax:
        ax.set_xlim(-10, 10)
...
def test_gate():
    ...
    with visualize(states, space, 'docs/gate.mp4') as _:
        pass
...
def test_walkway(n):
    ...
    path = 'docs/walkway_{}.png'.format(n)
    with socialforce.show.canvas(path) as ax:
        ...
        for ped in range(len(initial_state)):
            x = states[:, ped, 0]
            y = states[:, ped, 1]
            #print("ped " + str(ped) + ". x: " + str(x) + "\ny: " + str(y))
            print("ped " + str(ped) + ": " + str(states[:,ped,0:2]))
            ax.plot(x, y, '-o', label='ped {}'.format(ped), markersize=2.5)
        ax.legend()

    #with visualize(states, space, 'docs/walkway_{}.mp4'.format(n)) as _:
    #    _.set_xlim([-100,100])
    #    pass
